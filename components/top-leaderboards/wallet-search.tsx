...
interface UserData {
    rank: number;
    holder: string; // maps to "holder" in the response
    totalMint: number;
    houses: string[]; // will still store the array of house details
}
...
export default function CollectorSearch() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/fetchtopmint', {
                method: 'GET',
                headers: {
            ...
            if (response.ok) {
                ...
                // Ensure the walletAddress is compared correctly
                const filteredData = data.find(user => user.holder.toLowerCase() === walletAddress.toLowerCase());
                if (filteredData) {
                ...
                <div className="w-full mt-8">
                    <div className="relative p-[1px] lg:-mx-4 rounded-2xl">
                        ...
                        <div className="flex rounded-2xl p-2 lg:p-4 justify-between items-center relative bg-gradient-to-r from-[#693676] to-[#044895]">
                            <div className="flex items-center gap-4">
                                ...
                                <h1 className="lg:text-xl font-medium">{userData.holder}</h1>
                            </div>
                    ...
                    <div className="mt-4">
                        {userData.houses.map((house, index) => {
                            const houseDetails = house.split(', '); // Split the house details string
                            return (
                                <div key={index} className="bg-gray-800 p-4 rounded-lg mb-2">
                                    <h2 className="font-semibold">Contract Address: {houseDetails[0]}</h2>
                                    <h2 className="font-semibold">Contract Name: {houseDetails[1]}</h2>
                                    <h2 className="font-semibold">Points: {houseDetails[2]}</h2>
                                </div>
                            );
                        })}
                    </div>
